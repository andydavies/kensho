...
function originalInitiator(resourceURL,originalInitiator, additionalInitiatorData){
    // allways returns the original
    ...
    for (var key in expectedInitiators) {
        if(expectedInitiators[key]){
            if(expectedInitiators[key].expected && expectedInitiators[key].message){
                try{              
                    var retrievedInitiator = initiatorfunctionForTest('http://localhost:8888'+key, retrievedInitiators[key], additionalInitiatorData);

                    assert.deepEqual(retrievedInitiator,expectedInitiators[key].expected,expectedInitiators[key].message);
                    console.log(('INITIATOR SUCCES['+key +']: '+expectedInitiators[key].message).green );
                } catch (err){
                    if(isDeepEqualwithoutLineNumbers(retrievedInitiator,expectedInitiators[key].expected)) {
                        console.log(('INITIATOR PARTIAL SUCCES['+key +'] - correct URL with incorrect linenumber -: '+expectedInitiators[key].message).yellow );
                        if(retrievedInitiator){
                            console.log('actual'.yellow, JSON.stringify(retrievedInitiator).yellow);    
                        } else {
                        ...
                        if(retrievedInitiator){
                            console.log('actual'.red, JSON.stringify(retrievedInitiator).red);    
                        } else {
...
function prepareDebugger(){
	...
	chromeDevtools.onmessage = function(x){
...
exports.setInitiatorfunctionForTest = function(f){
    initiatorfunctionForTest = f;
};
var handlers = [];
exports.addEventHandler = function(eh){
    handlers.push(eh);
}

exports.run = function(){
    testWebserver.start();
    handlers.push(devtoolsEventhandler);
...
exports.getAdditionalInitiatorData = function(){return additionalInitiatorData;};